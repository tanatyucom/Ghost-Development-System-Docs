...
### 12A. Memory Candidate Review

Conversation Insight Detection後、Memory、Q、Repository Knowledge、Conversation
Insightのどれへ進めるか未確定だが、失うと困る会話由来Knowledgeがあるかを確認します。

```text
Memory Candidate: None
  -> Continue startup flow

Memory Candidate: Exists
  -> Notify briefly
  -> Check duplicate knowledge
  -> Decide Memory / Q / Repository Draft / Deferred / Reject / Duplicate later
```

MCはGDS Knowledge Inboxです。Canonical Knowledgeではなく、implementation、
commit、promotionの直接根拠にはしません。

Details follow `docs/rules/memory_candidate_rules.md` and
