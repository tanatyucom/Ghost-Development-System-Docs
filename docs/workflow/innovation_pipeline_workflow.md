...
Platform Standard Registry maps this flow to the following lifecycle:
...
`docs/architecture/platform_standard_registry.md`. Do not skip from Idea or
Prototype directly to Standard without validation evidence, a Platform
Promotion Decision Report, and Human Approval.
...
- Idea may become Candidate when reuse potential is reviewable.
- Idea may become Archived when it is rejected or no longer useful.

### Candidate

Platform標準候補として追跡します。

標準出力:

- source idea。
- target standard type。
...
- Prototype、Validation、Standard、またはArchivedの判断ができる。
...
判断例は `examples/platform_promotion_decision_report_examples.md` を参照します。

次へ進む条件:

- target location が明確。
- Human Approval Gate を通過している。
- Platform Standard Registry の Transition Matrix に従っている。
...
次の場合は昇格せず、Archived または Candidate に戻して扱います。
...
Archived にする場合は、archive reason と related report を残します。
