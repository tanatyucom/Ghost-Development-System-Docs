...
## Generated Documentation Output Rule

script が Markdown report や documentation artifact を生成する場合、本文説明は
日本語で出力します。

対象:

- generated report の目的説明。
- summary。
- result message。
- warning / error の説明。
...
- identifier。
- status value。
- script / workflow / check の固有名。
- 外部 command から返る未加工の diagnostic message。

生成後に手動翻訳が必要な report は、Knowledge Poka-Yoke の観点では未完成と
扱います。生成元の script 側で日本語出力を標準化します。
