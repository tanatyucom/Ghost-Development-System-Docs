...
### 2.1 Metrics / Observation Conflict Gate

Metrics と Human Observation、または Metrics と中間 Artifact が矛盾する場合は、
どちらかを即時に正しいと断定しません。

その候補、修正案、標準化案、または Production 採用判断を一度停止し、
Pipeline Trace、Expected State Check、First Broken Step Identification、
再検証を行います。

このGateの目的は、人間の印象を絶対視することではありません。
Metrics が見落としている処理単位、候補生成、抽出範囲、認識対象、責務境界の
不一致を調べることです。

Completion Report には最低限次を残します。

- 矛盾した metrics。
- Human Observation または artifact observation。
- 採用を停止した判断。
- 再調査した pipeline stage。
- 再検証結果。

### 2.2 Intermediate Artifact Acceptance Rule

Debug Artifact が期待状態を満たしていない場合、最終 metrics が良くても
成功判断にしてはいけません。

たとえば、候補画像、contact sheet、overlay、CSV、Markdown report が
「処理単位が違う」「対象行が違う」「候補が欠けている」「隣接要素が混入している」
ことを示す場合、まずDebug Artifact Reviewを完了し、必要ならDebug Escalation
Ladderへ進みます。
...
Candidate extraction、auto detection、OCR、recommendation、classification、
matching などで、候補生成と scoring / recognition が分かれている場合は、
scoring や recognition の調整前に、正しい候補が生成されているかを確認します。

正しい候補が存在しない場合、scoring 改善は根本解決になりません。
この確認はOCR固有ではなく、複数候補から選択する処理全般に適用します。
